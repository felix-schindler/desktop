...
public enum WorktreeOperations {
    ...
    nonisolated public static func listFromGitDirArgs(gitDir: String) -> [String] {
        ["--git-dir", gitDir, "worktree", "list", "--porcelain", "-z"]
    }

    nonisolated public static func addArgs(path: String, createBranch: String?, commitish: String?) -> [String] {
    ...
    nonisolated public static func removeArgs(worktreePath: String, force: Bool) -> [String] {
    ...
    nonisolated public static func moveArgs(oldPath: String, newPath: String) -> [String] {
    ...
    nonisolated public static func validateAdd(path: String, createBranch: String?, pathExists: Bool) -> String? {
    ...
    nonisolated public static func sortedForDisplay(_ entries: [WorktreeEntry]) -> [WorktreeEntry] {
    ...
    nonisolated public static func filter(_ entries: [WorktreeEntry], query: String) -> [WorktreeEntry] {
...
public enum WorktreeLiveOperations {
    nonisolated public static func listWorktrees(repositoryPath: String) async throws -> [WorktreeEntry] {
    ...
    nonisolated public static func listWorktreesFromGitDir(gitDir: String) async throws -> [WorktreeEntry] {
    ...
    nonisolated public static func addWorktree(repositoryPath: String, path: String, createBranch: String?, commitish: String?) async throws {
        let args = WorktreeOperations.addArgs(path: path, createBranch: createBranch, commitish: commitish)
        let result = try await GitProcess.run(args, workingDirectory: repositoryPath)
        if let error = classifyGitResult(result, args: args, successExitCodes: [0]) { throw error }
    }

    nonisolated public static func removeWorktree(repositoryPath: String, worktreePath: String, force: Bool = false) async throws {
        let args = WorktreeOperations.removeArgs(worktreePath: worktreePath, force: force)
        let result = try await GitProcess.run(args, workingDirectory: repositoryPath)
        if let error = classifyGitResult(result, args: args, successExitCodes: [0]) { throw error }
    }

    nonisolated public static func moveWorktree(repositoryPath: String, oldPath: String, newPath: String) async throws {
