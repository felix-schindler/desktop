...
public enum SubmoduleOperations {
    nonisolated public static func statusArgs() -> [String] {
        ["submodule", "status", "--"]
    }

    nonisolated public static func updateAfterOperationArgs(allowFileProtocol: Bool) -> [String] {
        var args: [String] = []
    ...
    nonisolated public static func resetPathsArgs(paths: [String]) -> [String] {
    ...
    nonisolated public static func parseStatus(_ stdout: String) -> [SubmoduleEntry] {
...
public enum LFSOperations {
    nonisolated public static func installGlobalArgs(force: Bool) -> [String] {
        var args = ["lfs", "install", "--skip-repo"]
        if force { args.append("--force") }
        return args
    }

    nonisolated public static func installRepoArgs(force: Bool) -> [String] {
        var args = ["lfs", "install"]
        if force { args.append("--force") }
        return args
    }

    nonisolated public static func trackJSONArgs() -> [String] {
        ["lfs", "track", "--json"]
    }

    nonisolated public static func checkAttrArgs(path: String) -> [String] {
        ["check-attr", "filter", path]
    }

    nonisolated public static let trackNoInstallHooksEnv = ["GIT_LFS_TRACK_NO_INSTALL_HOOKS": "1"]

    /// `git check-attr filter <path>` prints `… filter: lfs` when tracked.
    nonisolated public static func isTrackedOutput(_ stdout: String) -> Bool {
        stdout.range(of: ": filter: lfs") != nil
    }

    /// Minimal `git lfs track --json` shape: `{patterns: [{tracked: bool}]}`.
    nonisolated public static func isUsingLFS(json: String) -> Bool {
...
public enum SubmoduleLFSLiveOperations {
    nonisolated public static func listSubmodules(repositoryPath: String) async throws -> [SubmoduleEntry] {
    ...
    nonisolated public static func resetSubmodulePaths(repositoryPath: String, paths: [String]) async throws {
    ...
    nonisolated public static func updateSubmodules(repositoryPath: String, allowFileProtocol: Bool = false) async throws {
        let args = SubmoduleOperations.updateAfterOperationArgs(allowFileProtocol: allowFileProtocol)
        let result = try await GitProcess.run(args, workingDirectory: repositoryPath)
        if let error = classifyGitResult(result, args: args, successExitCodes: [0]) { throw error }
    }

    nonisolated public static func installLFSHooks(repositoryPath: String, force: Bool) async throws {
        let args = LFSOperations.installRepoArgs(force: force)
        let result = try await GitProcess.run(args, workingDirectory: repositoryPath)
        if let error = classifyGitResult(result, args: args, successExitCodes: [0]) { throw error }
    }

    nonisolated public static func installGlobalLFSFilters(force: Bool) async throws {
        let args = LFSOperations.installGlobalArgs(force: force)
        let result = try await GitProcess.run(args, workingDirectory: nil)
        if let error = classifyGitResult(result, args: args, successExitCodes: [0]) { throw error }
    }

    nonisolated public static func isUsingLFS(repositoryPath: String) async throws -> Bool {
    ...
    nonisolated public static func isTrackedByLFS(repositoryPath: String, path: String) async throws -> Bool {
