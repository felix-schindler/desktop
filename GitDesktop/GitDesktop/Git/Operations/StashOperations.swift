...
nonisolated public let desktopStashEntryMarker = "!!GitHub_Desktop"

/// Build the Desktop stash message for a branch. Port of `createDesktopStashMessage`.
nonisolated public func createDesktopStashMessage(branchName: String) -> String {
...
nonisolated public func extractBranchFromMessage(_ message: String) -> String? {
...
public struct StashLogRecord: Sendable, Equatable {
    ...
    nonisolated public init(name: String, stashSha: String, message: String, tree: String, parents: [String]) {
        self.name = name
...
public enum StashOperations {
    ...
    nonisolated public static let logFieldOrder = ["name", "stashSha", "message", "tree", "parents"]

    nonisolated public static var logFormatArgs: [String] {
        ["-z", "--format=%gD%x00%H%x00%gs%x00%T%x00%P"]
    }

    nonisolated public static func listArgs() -> [String] {
        ["log", "-g"] + logFormatArgs + ["refs/stash", "--"]
    }

    nonisolated public static func pushArgs(message: String) -> [String] {
        ["stash", "push", "-m", message]
    }

    nonisolated public static func popArgs(name: String) -> [String] {
        ["stash", "pop", "--quiet", name]
    }

    nonisolated public static func dropArgs(name: String) -> [String] {
        ["stash", "drop", name]
    }

    nonisolated public static func commitTreeArgs(parents: [String], message: String, tree: String) -> [String] {
    ...
    nonisolated public static func storeArgs(message: String, commitID: String) -> [String] {
        ["stash", "store", "-m", message, commitID]
    }

    nonisolated public static func showFilesArgs(stashSha: String) -> [String] {
        ["stash", "show", stashSha, "--raw", "--numstat", "-z",
         "--format=format:", "--no-show-signature", "--"]
    }

    /// Parse NUL-delimited stash log output into records.
    nonisolated public static func parseLogRecords(_ output: Data) -> [StashLogRecord] {
    ...
    nonisolated public static func desktopEntries(from records: [StashLogRecord]) -> [StashEntry] {
    ...
    nonisolated public static func totalCount(recordCount: Int) -> Int {
        max(0, recordCount - 1)
    }
    ...
    nonisolated public static func pushSucceeded(exitCode: Int32, stderr: String) -> Bool {
    ...
    nonisolated public static func pushHadNoChanges(stdout: String) -> Bool {
...
public enum StashLiveOperations {
    ...
    nonisolated public static func getStashes(repositoryPath: String) async throws -> (desktopEntries: [StashEntry], totalCount: Int) {
    ...
    nonisolated public static func createStash(repositoryPath: String, branchName: String) async throws -> Bool {
    ...
    nonisolated public static func entry(matchingSha sha: String, in entries: [StashEntry]) -> StashEntry? {
        entries.first { $0.stashSha == sha }
    }

    /// Port of `dropDesktopStashEntry` (no-op when the SHA is unknown).
    @discardableResult
    nonisolated public static func dropStash(repositoryPath: String, entries: [StashEntry], stashSha: String) async throws -> Bool {
    ...
    nonisolated public static func popStash(repositoryPath: String, entries: [StashEntry], stashSha: String) async throws -> PopResult {
    ...
    nonisolated public static func moveStashEntry(repositoryPath: String, entry: StashEntry, toBranch branchName: String) async throws {
    ...
    nonisolated public static func stashedFiles(repositoryPath: String, stashSha: String) async throws -> [CommittedFileChange] {
